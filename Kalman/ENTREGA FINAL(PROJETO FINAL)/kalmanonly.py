...
while(True):
    ...
    delta_y = box_cam2[1][0] - py

    vx = delta_x/tempo
    vy = delta_y/tempo

    previsaox = box_cam2[0][0] + vx * tempo
    previsaoy = box_cam2[1][0] + vy * tempo

    print(previsaox,previsaoy)
    ...
    KK = (HH*sig1**2)/(HH**2*sig1**2 + sig2**2)

    mi_fundida_X = mi1 + KK * (mi2 - HH*mi1)

    var_fundida_X = sig1**2 - KK*HH*sig1**2

    sig_fundida_X = var_fundida_X**0.5
    ...
    mi_fundida_Y = mi1 + KK * (mi2 - HH*mi1)

    var_fundida_Y = sig1**2 - KK*HH*sig1**2

    sig_fundida_Y = var_fundida_Y ** 0.5


    cv2.circle(img1bgr,(int(mi_fundida_X),int(mi_fundida_Y)), 8, (255,0,0), -1)

    cv2.circle(img1bgr,(int(previsaox),int(previsaoy)), 8, (127,0,127), -1)
